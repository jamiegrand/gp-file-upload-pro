...
class GP_File_Upload_Pro extends GP_Plugin {
	...
	public function localize_frontend_scripts( $form ) {
		...
		wp_localize_script( 'gp-file-upload-pro', 'GPFUP_AJAX_URL', admin_url( 'admin-ajax.php' ) );
		wp_localize_script( 'gp-file-upload-pro', 'GPFUP_NONCE', wp_create_nonce( 'gpfup' ) );

		wp_localize_script( 'gp-file-upload-pro', 'GPFUP_STRINGS', array(
	...
	public function field_settings_ui( $position ) {
		...
		<li class="gpfup-field-setting field_setting" style="display:none;">
			<input type="checkbox" value="1" id="gpfup-enable" onchange="SetFieldProperty( 'gpfupEnable', this.checked );" />
        ...
        <div class="gp-child-settings gpfup-child-settings gpfup-show-if-multiple">
            <li>
                <input type="checkbox" value="1" id="gpfup-enable-crop" onchange="SetFieldProperty( 'gpfupEnableCrop', this.checked );" />
		...
		<script type="">
            ( function( $ ) {
                ...
                $( document ).bind( 'gform_load_field_settings', function( event, field, form ) {
                    ...
                    $enable.prop( 'checked', field['gpfupEnable'] == true );
                    $enable.prop( 'disabled', !multipleFiles );

                    $('#gpfup-enable-crop').prop( 'checked', field['gpfupEnableCrop'] == true );
