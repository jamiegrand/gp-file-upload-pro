<template>
    <div :class="{ 'gpfup__preview': true, 'gpfup__preview--crop': croppingAvailable }">
        ...
        <img v-else-if="imgSrc" :src="imgSrc" :style="previewSize" @load="storePreviewDimensions(file)" ref="image" />
        <div v-else-if="!isImage()" class="gpfup__file-icon">
...
<script lang="ts">
    ...
    export default Vue.extend({
        name: "ImagePreview",
        props: ['file', 'error', 'enableCrop', 'onlyImagesAllowed'],
        computed: {
		...
		methods: {
			...
			storePreviewDimensions: function(file: MOxieFile) : void {
            	this.$store.state.storage.storePreviewDimensions(file.id, {
            		width: (this.$refs.image as Element).clientWidth,
            		height: (this.$refs.image as Element).clientHeight,
				});
			}
        }
    });
